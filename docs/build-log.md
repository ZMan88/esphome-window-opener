...
**Fix.** Vendored the component locally (`firmware/components/{stepper,tmc2209,tmc2209_hub}/`, from slimcdk commit `b5f0e44`) and changed the `micros()` timestamp types (`last_calculation_`, `last_step_`, and the `now` args) from `time_t` to wrap-safe `uint32_t`, mirroring core `esphome/components/stepper`. The bug is unfixed on upstream master (`b5f0e44` *is* master HEAD), so bumping the ref would not have helped — hence vendoring. `firmware/variants/stepper-uart.yaml` now points `external_components` at `type: local, path: components` instead of GitHub.

**Upstreamed.** The same fix was submitted to the component author as [slimcdk/esphome-custom-components#50](https://github.com/slimcdk/esphome-custom-components/pull/50) (branch `ZMan88:fix/stepper-micros-wrap`). When it merges, drop the vendored copy and go back to `external_components: github://…` pinned at/after the merge commit — the vendored tree is identical to that PR. See `firmware/components/README.md`.
