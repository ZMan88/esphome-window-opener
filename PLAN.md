...
- 2026-06-05 — **Firmware feature pass on `stepper-uart.yaml`** (now the active variant in `window-opener.yaml`):
  ...
  - **Free-wheel** (`switch.window_freewheel`): releases the coils (`tmc2209.currents` `standstill_mode: freewheeling`, 0 mA hold) so the sash is hand-movable. (Avoids `tmc2209.enable/disable`, whose slimcdk `ActivationAction` fails to compile under ESPHome's 2026-04 TemplatableFn change.) Move-detection: a transition of the closed signal while free-wheeling marks position unknown → next command re-homes first. Limitation: a bare TMC2209 has no rotor feedback, so hand-movement that never crosses the closed sensor can't be detected (keeps last position). Encoder would fix this — deferred.
  ...
  - **Switchable end-detection** (added 2026-06-06): `homing_mode: contact | sensorless` substitution (in `window-opener.yaml`). `contact` = home closed from the HA contact, open by calibrated step count (StallGuard = obstruction backstop). `sensorless` = no contact: ram closed at a gentle `homing_current` until StallGuard trips, and *learn* the open end from the opening-direction stall into a runtime `g_full_stroke`. Sensorless requires a tuned `stall_threshold` (SGTHRS > 0). Kept switchable so both can be compared on the real rig before committing — sensorless rams the stops every home (wear) and can't distinguish obstruction from end, so contact is the default/recommended. Position % is relative to `g_full_stroke` (fixed in contact, learned in sensorless — do one full open after homing to learn it).
